...
cdef class SA1(Board):
    ...
    def extra_state(self):
        """The SA-1's own processor and the clock it runs on.  Without these a
        state restores the console around a second CPU still executing
        whatever it had reached."""
        return {"cpu": (self.cpu.state_ints(), self.cpu.state_blobs()),
                "clock": self.space.master_clock,
                "target": self.space.target}

    def load_extra(self, d):
        ints, blobs = d["cpu"]
        self.cpu.load_ints(ints)
        self.cpu.load_blobs(blobs)
        self.space.master_clock = d["clock"]
        self.space.target = d["target"]
    ...
    def load_ints(self, v):
        ...
        self.sa1_nmi = v[13]
        self.scpu_irq = v[14]
        self.dma_irq_scpu = v[15]
        self.dma_irq_sa1 = v[16]
        self.timer_irq = v[17]
        self.stopped = v[18]
        self.bmaps = v[19]
        self.bmap = v[20]
    ...
    def state_blobs(self):
        return [PyBytes_FromStringAndSize(<char *>self.iram, 2048)]

    def load_blobs(self, blobs):
        if len(blobs[0]) != 2048:
            raise ValueError('bad iram blob')
        memcpy(<char *>self.iram, <char *><bytes>blobs[0], 2048)
