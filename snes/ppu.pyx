...
cdef class PPU:
    ...
    cdef void write_reg(self, uint32_t addr, uint8_t value) noexcept:
        ...
        if reg == 0x00:                                   # INIDISP
            ...
            # Writing the register starts a fresh block from this line.
            if self.render_row >= 0:
                self.mosaic_start = self.render_row
    ...
    cdef inline int _mosaic_y(self, int bg, int line) noexcept:
        """The row a mosaic block is sampled from.

        Vertically this is a counter rather than a division: the block runs
        for as many lines as the size said when it started, so changing the
        size part-way down the screen lets the current block finish first.
        The counter restarts at the top of the frame and on a write to
        $2106, which is what the hardware does."""
        if not self.mosaic_enable[bg] or self.mosaic_size == 0:
            return line
        return self.mosaic_start
    ...
    def load_ints(self, v):
        ...
        self.ppu1_mdr = v[55]
        self.ppu2_mdr = v[56]
        self.mosaic_start = v[57]
        self.mosaic_left = v[58]
        for i in range(4):
