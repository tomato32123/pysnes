...
cdef class SuperFX(Board):
    ...
    def extra_state(self):
        """The work RAM.  Its size comes from the cartridge, so it cannot be a
        generated blob -- those are fixed-size memcpys."""
        return bytes(self.ram_data)

    def load_extra(self, blob):
        n = min(len(blob), len(self.ram_data))
        self.ram_data[:n] = blob[:n]

    # -- save state (generated by tools/gen_state.py; do not edit) --------

    def state_ints(self):
        cdef int i, j
        v = [self.r14_modified, self.r15_modified, self.sfr, self.pbr, self.rombr, self.rambr, self.cbr, self.scbr, self.colr, self.bramr, self.vcr, self.clsr, self.pipeline, self.ramaddr, self.scmr_ht, self.scmr_ron, self.scmr_ran, self.scmr_md, self.por_obj, self.por_freezehigh, self.por_highnibble, self.por_dither, self.por_transparent, self.cfgr_irq, self.cfgr_ms0, self.sreg, self.dreg, self.romcl, self.romdr, self.ramcl, self.ramar, self.ramdr, self.rom_mask, self.ram_mask, self.gsu_clock, self.target]
        for i in range(16):
            v.append(self.r[i])
        for i in range(512):
            v.append(self.cache_buffer[i])
        for i in range(32):
            v.append(self.cache_valid[i])
        for i in range(2):
            v.append(self.pc_offset[i])
        for i in range(2):
            v.append(self.pc_bitpend[i])
        for i in range(2):
            for j in range(8):
                v.append(self.pc_data[i][j])
        return v

    def load_ints(self, v):
        cdef int i, j, k = 36
        self.r14_modified = v[0]
        self.r15_modified = v[1]
        self.sfr = v[2]
        self.pbr = v[3]
        self.rombr = v[4]
        self.rambr = v[5]
        ...
        self.scmr_md = v[17]
        self.por_obj = v[18]
        self.por_freezehigh = v[19]
        self.por_highnibble = v[20]
        self.por_dither = v[21]
        self.por_transparent = v[22]
        self.cfgr_irq = v[23]
        self.cfgr_ms0 = v[24]
        self.sreg = v[25]
        self.dreg = v[26]
        ...
        k += 16
        for i in range(512):
            self.cache_buffer[i] = v[k + i]
        k += 512
        for i in range(32):
            self.cache_valid[i] = v[k + i]
        k += 32
        for i in range(2):
            self.pc_offset[i] = v[k + i]
        k += 2
        for i in range(2):
            self.pc_bitpend[i] = v[k + i]
        k += 2
        for i in range(2):
            for j in range(8):
                self.pc_data[i][j] = v[k + i * 8 + j]
        k += 16
